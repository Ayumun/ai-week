---
description: "GitHub Issueのタスクグループを実行するコマンド。ARGUMENTS: Issue番号（必須、#123形式）とタスクグループ番号（必須、1.1形式、複数指定可：1.1 1.2 1.3）"
...
- **タスクグループ番号**（必須、1つ以上。例: `1.1`、`1.1 1.2 1.3`、`2.1 2.2`）

どちらかが指定されていない場合は、ユーザーに入力を要求します。
複数のタスクグループが指定された場合、番号順に自動実行します。
...
- **プッシュ前チェック**: `npm run prepush` を必ず実行し、全チェックを通過してからプッシュ

**禁止事項**:
- まとめコミット
- 複数目的の混在コミット
- コミット粒度の過剰肥大化
- `npm run prepush` 未実行でのプッシュ
